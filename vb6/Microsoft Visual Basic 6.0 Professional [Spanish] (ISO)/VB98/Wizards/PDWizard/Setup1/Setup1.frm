...
'
    Const strINI_FILES$ = "Setup1 Files"                           'default section to install
    ...
    gsCABNAME = gstrWinDir & ReadIniFile(gstrSetupInfoFile, gstrINI_BOOT, gstrINI_CABNAME)
    ...
    If fCreateGroup Then
        For iLoop = 0 To cGroups - 1
            strGroupName = ""
            strGroupName = frmGroup.GroupName(frmSetup1, GetGroup(gsICONGROUP, iLoop))
    ...
    End If
    '
    ' If we installed mdac_typ.EXE, we now need to run it
    ' so it will install any additional files it contains.
    '
    If gfMDag = True Then
        If FileExists(gstrMDagInstallPath) Then
            Shell gstrMDagInstallPath & gstrFILE_MDAGARGS
        End If
    End If
    '
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If FileExists(GetShortPathName(gsCABNAME)) Then Kill GetShortPathName(gsCABNAME)
End Sub
